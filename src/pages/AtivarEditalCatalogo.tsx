...
export function AtivarEditalCatalogo() {
  ...
  React.useEffect(() => {
    if (!deferredSearch || !listQuery.data || lastTrackedSearch.current === deferredSearch) return;
    lastTrackedSearch.current = deferredSearch;
    trackTelemetry("catalog_search_started", {
      has_filters: true,
      result_count: listQuery.data.total,
    });
