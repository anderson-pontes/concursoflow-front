  ...
  return (
    <div className="space-y-5 pb-10">
      ...
      <section className="overflow-hidden rounded-xl border border-border bg-card shadow-sm">
        <div className="flex items-center justify-between border-b border-border p-4"><div><h2 className="font-semibold">4. Disciplinas do concurso</h2><p className="text-xs text-muted-foreground">{cargos.reduce((sum, cargo) => sum + cargo.disciplinas.length, 0)} disciplinas{cargos.length > 1 ? ` em ${cargos.length} cargos` : selectedCargo ? ` para ${selectedCargo.nome}` : ""}</p></div>{editable ? <Button variant="outline" className="min-h-11 gap-2" onClick={addCargo}><Plus /> Adicionar outro cargo</Button> : null}</div>
