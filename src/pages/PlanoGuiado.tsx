...
export function PlanoGuiado() {
  ...
  const [catalogSelectionValidation, setCatalogSelectionValidation] = React.useState<"idle" | "validating" | "valid" | "error">("idle");
  ...
  const canNext = step === 1 ? Boolean(tipo) : step === 2 ? (tipo === "catalogo" ? Boolean(cargoId && catalogSelectionValidation === "valid") : Boolean(nome.trim() && orgao.trim())) : step === 3 ? disciplinas.some((d) => d.ativa) : step === 4 ? Object.values(config.disponibilidade_minutos).some((m) => m > 0) && config.sessao_max_minutos >= config.sessao_min_minutos : true;
  const next = () => {
    if (!tipo) return;
