      ...
      <Table>
        ...
        <TableBody>{items.map((item) => {
          ...
          return <TableRow key={item.id} className={cn(selected && "bg-primary-muted")}>
            ...
            <TableCell><Badge variant="secondary">Publicado</Badge><span className="mt-1 block text-xs text-muted-foreground">Versão {item.versao_atual?.numero ?? "publicada"}{item.versao_atual?.data_prova ? ` · prova ${formatDate(item.versao_atual.data_prova)}` : ""}</span><CatalogClassificationSummary item={item} /></TableCell>
...
function CatalogClassificationSummary({ item }: { item: EditalCatalogo }) {
  const classification = item.versao_atual?.classificacao;
  if (!classification) return null;
  const values = [classification.esfera?.nome, classification.ano_edital ? `Edital ${classification.ano_edital}` : null, ...classification.areas.slice(0, 2).map((area) => area.nome)].filter((value): value is string => Boolean(value));
  if (!values.length) return <span className="mt-2 block text-xs text-muted-foreground">Classificação não informada</span>;
  return <div className="mt-2 flex flex-wrap gap-1.5" aria-label="Classificação do edital">{values.map((value) => <Badge key={value} variant="outline" className="font-normal">{value}</Badge>)}</div>;
}
