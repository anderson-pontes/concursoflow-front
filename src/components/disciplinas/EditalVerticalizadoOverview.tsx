...
export function EditalVerticalizadoOverview({ concursoId }: { concursoId: string }) {
  ...
  return <section aria-labelledby="edital-verticalizado-title" className="space-y-4">
    ...
    {noDisciplines ? <EmptyState title="Nenhuma disciplina neste concurso" description="Vincule disciplinas e tópicos ao concurso para visualizar o edital verticalizado." /> : noTopics ? <EmptyState title="Disciplinas sem tópicos" description="Cadastre tópicos nas disciplinas vinculadas para acompanhar o progresso do edital." /> : items.length === 0 ? <EmptyState title="Nenhum tópico encontrado" description="Ajuste a busca ou limpe os filtros para ampliar os resultados." action={<Button variant="outline" onClick={clearFilters}>Limpar filtros</Button>} /> : isDesktop ? <DesktopTable items={items} returnTo={returnTo} returnState={{ editalContestId: concursoId, editalSearch: appliedSearch, editalPageCount: edital.data?.pages.length ?? 1 }} /> : <MobileCards items={items} expanded={expanded} onExpand={(id) => updateParam("expandida", expanded === id ? undefined : id, true)} returnTo={returnTo} returnState={{ editalContestId: concursoId, editalSearch: appliedSearch, editalPageCount: edital.data?.pages.length ?? 1 }} />}
...
function useMediaQuery(query: string) { const [matches, setMatches] = React.useState(() => typeof window !== "undefined" && window.matchMedia(query).matches); React.useEffect(() => { const media = window.matchMedia(query); const listener = () => setMatches(media.matches); listener(); media.addEventListener("change", listener); return () => media.removeEventListener("change", listener); }, [query]); return matches; }
